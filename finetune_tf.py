    ...
    def _resource_apply_dense(self, grad, var, apply_state=None):
        ...
        if self.has_beta_1:
            ...
            new_m = coefficients["beta1"] * \
                tf.to_float(m) + (1.0 - coefficients["beta1"]) * subtrahend
            subtrahend = new_m
...
class AdamWeightDecay(tf.keras.optimizers.Adam):
  ...
  """
    ...
    def apply_gradients(self, grads_and_vars, clip_norm=1.0, name=None):
...
def create_optimizer_adafactor(init_lr, num_train_steps, num_warmup_steps):
    """Creates an optimizer with learning rate schedule."""
    # Implements linear decay of the learning rate.
    learning_rate_fn = tf.keras.optimizers.schedules.PolynomialDecay(
        initial_learning_rate=init_lr, decay_steps=num_train_steps, end_learning_rate=0.0
    ...
    optimizer = AdafactorOptimizer(learning_rate=learning_rate_fn)
    return optimizer
...
def get_dataset():
    ...
    train_n_original_tokens = 0
    train_n_tokens = 0

    val_n_original_tokens = 0
    val_n_tokens = 0

    with open('./data/wikitext-2-raw/wiki.train.raw', encoding="utf-8") as handle:
        text = handle.read()
        train_n_original_tokens += len(text.strip().split(" "))
    ...
    if len(tokenized_text) < 1024:
        ...
        for i in range(len(tokenized_text) // 1024):
            example = tokenizer.build_inputs_with_special_tokens(
                tokenized_text[i * 1024: (i + 1) * 1024])
            inputs.append(example[:-1])
            labels.append(example[1:])

    train_n_tokens += sum([len(batch) for batch in inputs])

    train_dataset = tf.data.Dataset.from_tensor_slices((inputs, labels))
    train_dataset = train_dataset.shuffle(100).batch(8, drop_remainder=True)

    with open('./data/wikitext-2-raw/wiki.valid.raw', encoding="utf-8") as handle:
        text = handle.read()
        val_n_original_tokens += len(text.strip().split(" "))
    ...
    val_n_tokens += sum([len(batch) for batch in inputs])

    val_dataset = tf.data.Dataset.from_tensor_slices((inputs, labels))
    val_dataset = val_dataset.shuffle(100).batch(8, drop_remainder=True)
...
def create_model():
    ...
    # return TFGPT2LMHeadModel.from_pretrained('./', from_pt=True)
    return TFGPT2LMHeadModel.from_pretrained('distilgpt2')


def train_adj_loss(y_true, y_pred):
    cross_entropy = K.sparse_categorical_crossentropy(
        y_true, y_pred, from_logits=True)

    cross_entropy *= ((2417664 - 1) / (2088674 - 1))

    return cross_entropy


def val_adj_loss(y_true, y_pred):
    cross_entropy = K.sparse_categorical_crossentropy(
        y_true, y_pred, from_logits=True)

    cross_entropy *= ((249856 - 1) / (217646 - 1))
...
def main():
    ...
    optimizer = AdafactorOptimizer(learning_rate=5e-5)
    ...
    n_train_steps = int(len(list(train_dataset)) / 8)
    # optimizer = create_optimizer_adafactor(
    # 5e-5, n_train_steps, int(0.1 * n_train_steps))

    wandb_callback = WandbCallback()

    lr_callback = WarmUpCosineDecayScheduler(
        learning_rate_base=5e-5, total_steps=n_train_steps, warmup_steps=int(0.1 * n_train_steps))
    ...
    model.compile(optimizer=optimizer, loss=[
                  loss, *[None] * model.config.n_layer], metrics=[train_adj_loss, val_adj_loss])

    model.fit(train_dataset, validation_data=val_dataset,
              epochs=1, callbacks=[wandb_callback, lr_callback])
    ...
    model.save_pretrained(wandb.run.dir)
